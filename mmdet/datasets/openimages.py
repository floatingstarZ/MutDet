...
class OpenImagesDataset(CustomDataset):
    ...
    def get_ann_info(self, idx):
        ...
        gt_is_group_ofs = np.array(is_group_ofs, dtype=np.bool_)

        # These parameters is not used yet.
        is_occludeds = np.array(is_occludeds, dtype=np.bool_)
        is_truncateds = np.array(is_truncateds, dtype=np.bool_)
        is_depictions = np.array(is_depictions, dtype=np.bool_)
        is_insides = np.array(is_insides, dtype=np.bool_)
    ...
    def add_supercategory_ann(self, annotations):
        ...
        for i, ann in enumerate(annotations):
            ...
            annotations[i] = dict(
                ...
                gt_is_group_ofs=np.array(gt_is_group_ofs).astype(np.bool_))
