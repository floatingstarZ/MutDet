...
from AlignRotate.datasets.multi_custom import MultiCustomDataset
...
class MultiDOTADataset(MultiCustomDataset):
    ...
    def __init__(self,
                 ann_file_list,   # annotations folder
                 ex_ann_file_list,  # object embeddings folder
                 img_prefix_list, # image folders
                 save_dir,
                 pipeline,
                 version='oc',
                 difficulty=100,
        ...
        super(MultiDOTADataset, self).__init__(ann_file_list, ex_ann_file_list, img_prefix_list, save_dir,
                                               pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder, img_prefix, ex_ann_folder):
        """
            Args:
        ...
        if not ann_files:  # test phase
            ...
            for ann_file in ann_files:
                ...
                data_info['filename'] = img_prefix + '/' + img_name
                data_info['ann'] = {}
            ...
            for ann_file in ann_files:
                ...
                ###########
                if ex_ann_folder != '':
                    ex_ann_pth = ex_ann_folder + '/' + Path(ann_file).stem + '.pkl'
                ...
                for k, v in data_info['ann'].items():
                    if len(v) != n_gt:
                        f_name = data_info['filename']
                        print(f'Warning, in {f_name}, num({k}): {len(v)} does not match n_gt: {n_gt}')
                        new_v = []
                        ...
                        print(f'Warning, in {f_name}, num({k}): {len(v)} is changed to {len(new_v)}')
        ...
        img_ids = [*map(lambda x: x['filename'], data_infos)]
