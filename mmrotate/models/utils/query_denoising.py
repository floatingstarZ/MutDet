...
class DnQueryGenerator:
    ...
    def __call__(self,
        ...
        # ----------------- num_groups（组数量）会根据每张图片进行动态调整
        # ----------------- 实例很多，则只分配一组，否则按照 100(num_dn) // num_instance计算
        ...
        # ----------------- 按照self.label_noise_scale * 0.5进行随机label扰乱
...
class DnQueryGeneratorCustom:
    ...
    def __init__(self,
                 ...
                 group_cfg=dict(
                     dynamic=True, num_groups=None, num_dn_queries=None)):
        super(DnQueryGeneratorCustom, self).__init__()
        self.num_queries = num_queries
        self.hidden_dim = hidden_dim
        self.num_classes = num_classes
        self.label_noise_scale = noise_scale['label']
