...
class CustomDOTADataset(DOTADataset):
    def __init__(self,
                 *args,
                 ...
                 # ------- 额外信息的pkl（例如特征）
                 ex_ann_folder='',
                 format_score_thr=0.05,
                 format_pre_iou_thr=0.5,
                 **kwargs):
        self.eval_ious = eval_ious
        self.ex_ann_folder = ex_ann_folder
        self.format_score_thr = format_score_thr
        self.format_pre_iou_thr = format_pre_iou_thr
        super(CustomDOTADataset, self).__init__(*args, **kwargs)
    ...
    def load_annotations(self, ann_folder):
        ...
        if not ann_files:  # test phase
            ...
            for ann_file in ann_files:
                ...
                data_info['ann'] = {}
                ###########
                if self.ex_ann_folder != '':
                    ex_ann_pth = self.ex_ann_folder + '/' + Path(ann_file).stem + '.pkl'
                    ex_infos = pklload(ex_ann_pth, msg=False)
                    data_info['ann'].update(ex_infos)
                ###########
                gt_bboxes = []
                gt_labels = []
                gt_polygons = []
                ...
                # 对ex info进行截取，使得数量一致
                n_gt = len(gt_bboxes)
                for k, v in data_info['ann'].items():
                    if len(v) != n_gt:
                        print(f'Warning, n_ex_info {len(v)} does not match n_gt {n_gt}')
                        new_v = []
                        for i, if_keep in enumerate(keep_inds):
                            if if_keep:
                                new_v.append(v[i])
                        new_v = np.array(v)
                        data_info['ann'][k] = new_v
    ...
    def evaluate(self,
        ...
        if metric == 'mAP':
            ...
            for iou in self.eval_ious:
                print_log(f'\n{"-" * 15}iou_thr: {iou}{"-" * 15}', logger=logger)
    ...
    def merge_det(self, results, nproc=4):
        ...
        # ---- 对每张子图像
        print(f'Collect All Results, with format_pre_iou_thr: {self.format_pre_iou_thr}, format_score_thr: {self.format_score_thr}')
        for idx in tqdm(range(len(self))):
            ...
            for i, dets in enumerate(result):
                ######检测结果预处理
                scores = dets[:, -1]
                score_keep_ids = scores >= self.format_score_thr
                if np.sum(score_keep_ids) == 0:
                    new_result.append(np.zeros((0, 7)))
                    continue
                dets = dets[score_keep_ids]
                try:
                    cls_dets = torch.from_numpy(dets).cuda()
                except:  # noqa: E722
                    cls_dets = torch.from_numpy(dets)
                nms_dets, nms_keep_ids = nms_rotated(cls_dets[:, :5], cls_dets[:, -1],
                                                     self.format_pre_iou_thr)
                nms_keep_ids = nms_keep_ids.detach().cpu().numpy()
                dets = dets[nms_keep_ids]
                ######
        ...
        merged_results = []
        for i, info in tqdm(list(enumerate(list(collector.items())))):
            results = _merge_func(info, self.CLASSES, iou_thr=0.1)
    ...
    for i in range(len(CLASSES)):
        if len(dets[labels == i]) == 0:
            ...
            try:
                ...
                print(img_id, i, len(dets[labels == i]))
                cls_dets = torch.from_numpy(dets[labels == i])
            # print(img_id, i, len(cls_dets))
